...
func SimulateRouting(ctx context.Context, group, prompt, strategy string) (QuizResult, error) {
	...
	scored := ScoreModelProfilesWithFeatures(features, candidates.Models, candidates.Profiles, candidates.Availability, policy)
	...
	return QuizResult{
		...
		AvailableModels: candidates.Models, FilteredOutModels: candidates.FilteredOut,
		FilterReasons: candidates.FilterReasons, Strategy: strategy, TurnType: turnType.String(), Difficulty: DetectTaskDifficulty(features).String(),
	}, nil
}
...
func buildQuizResult(prompt string, available, candidates []string) QuizResult {
	...
	return QuizResult{
		...
		Difficulty:        DetectTaskDifficulty(features).String(),
	}
}
